            ...
            <el-row>
                <el-col :span="15" :offset="1" class="input">
                    <div class="box" v-for="(item,index) in input" :key="index">
                        ...
                        <input type="text" class="font14 inputpadd" :placeholder="item.placeholder" v-model="item.value"Z :readonly='item.id=="freightCostTemplate"' :maxlength="item.id==2?6:999999" @keyup="keyCategories(item.id,index,item.name)" @blur="blueChange(item.value,index,item.id)" @focus="focusChange(item.id,index,item.name)">
  ...
  methods: {
    ...
    deleteattributes(index, id) {
      ...
      if (yon) {
        ...
        this.$message.error("使用运费模板属性的商品，必须需要商品重量属性");
      }
    },
